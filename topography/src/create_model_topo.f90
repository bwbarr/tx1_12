...
program create_model_topo
  ...
  real(kind=dbl_kind), dimension(:,:), allocatable :: TOPO_Z
  real(kind=real_kind), dimension(:), allocatable :: INDAT_R1D ! Input data buffers
  real(kind=dbl_kind), dimension(:), allocatable :: INDAT_D1D
  real(kind=real_kind), dimension(:,:), allocatable :: INDAT_R2D ! Input data buffers
  real(kind=dbl_kind), dimension(:,:), allocatable :: INDAT_D2D
  ...
  integer :: type_in_lon, type_in_lat, type_in_z
  integer :: dimid_lonh, dimid_lath, dimid_lonq, dimid_latq 
  integer, dimension(2) :: indcnt, indstr, dims
  character(char_len) :: attr_name, attr_valc
  integer :: vid_ofrac, vid_mask, &
  ...
  !---- Allocate data arrays with expanded dimensions
  print *,'Allocating input TOPO arrays'
  allocate(TOPO_LON(0:nx_topo+1))
  allocate(TOPO_LAT(0:ny_topo+1))
  allocate(TOPO_Z(0:nx_topo+1,0:ny_topo+1),stat=ierr)
  if ( ierr /= 0 ) then
  ...
  ierr = nf_inq_vartype(ncid,vid,type_in_lon)
  print *,'vartype ierr = ',ierr,'input longitude type = ',type_in_lon
 
  indstr = (/1,0/)
  indcnt = (/nx_topo,0/)
  select case (type_in_lon)
  case (NF_FLOAT)
     print *,' Getting real longitude'
     allocate(INDAT_R1D(nx_topo))
     ierr = nf_get_vara_real(ncid,vid,indstr,indcnt,INDAT_R1D)
     TOPO_LON(1:nx_topo) = INDAT_R1D
     deallocate(INDAT_R1D)
  case (NF_DOUBLE)
     print *,'Getting double longitude coordinate'
     allocate(INDAT_D1D(nx_topo))
     ierr = nf_get_vara_double(ncid,vid,indstr,indcnt,INDAT_D1D)
     TOPO_LON(1:nx_topo) = INDAT_D1D
     deallocate(INDAT_D1D)
  case DEFAULT
     print *,'longitude coordinate input type not found'
     stop
  end select
  if ( ierr /= 0 ) then
  ...
  TOPO_LON(0) = TOPO_LON(nx_topo)-360.0d0
  TOPO_LON(nx_topo+1) = TOPO_LON(1)+360.0d0
  dlon_topo = (TOPO_LON(nx_topo)-TOPO_LON(1))/nx_topo  ! assumes periodic point is included in data

  if ( verbose ) then
     write(*,"(a,2f16.7)") 'Extended TOPO LON min/max=',minval(TOPO_LON),maxval(TOPO_LON)
     write(*,"(a,f20.10)") 'dlon_topo = ',dlon_topo,' degrees'
     write(*,"(a,4f12.5,a,4f12.5)") &
  ...
  ierr = nf_inq_vartype(ncid,vid,type_in_lat)
  print *,ierr,'input latitude type = ',type_in_lat

  indstr = (/1,0/)
  indcnt = (/ny_topo,0/)
  select case (type_in_lat)
  case (NF_FLOAT)
     print *,' Getting real latitude coordinate'
     allocate(INDAT_R1D(ny_topo))
     ierr = nf_get_vara_real(ncid,vid,indstr,indcnt,INDAT_R1D)
     TOPO_LAT(1:ny_topo) = INDAT_R1D
     deallocate(INDAT_R1D)
  case (NF_DOUBLE)
     print *,'Getting double latitude coordinate'
     allocate(INDAT_D1D(ny_topo))
     ierr = nf_get_vara_double(ncid,vid,indstr,indcnt,INDAT_D1D)
     TOPO_LAT(1:ny_topo) = INDAT_D1D
     deallocate(INDAT_D1D)
  case DEFAULT
  ...
  dlat_topo = (TOPO_LAT(ny_topo)-TOPO_LAT(1))/(ny_topo-1)
  ...
  print *,' input data z ncid = ',ncid,' vid = ',vid
  ierr = nf_inq_vartype(ncid,vid,type_in_z)
  print *,ierr,'input depth variable type = ',type_in_z
  
  indstr = (/1,1/)
  indcnt = (/nx_topo,ny_topo/)
  select case (type_in_z)
  case (NF_FLOAT)
     print *,' Getting real topo data'
     allocate(INDAT_R2D(nx_topo,ny_topo),stat=ierr)
     ierr = nf_get_vara_real(ncid,vid,indstr,indcnt,INDAT_R2D)
     TOPO_Z(1:nx_topo,1:ny_topo) = INDAT_R2D
     deallocate(INDAT_R2D)
  case (NF_DOUBLE)
     print *,'Getting double topo data'
     allocate(INDAT_D2D(nx_topo,ny_topo),stat=ierr)
     ierr = nf_get_vara_double(ncid,vid,indstr,indcnt,INDAT_D2D)
     TOPO_Z(1:nx_topo,1:ny_topo) = INDAT_D2D
     deallocate(INDAT_D2D)
  case DEFAULT
     print *,'input type not found'
     stop
  end select
  ...
  print *,' Read original topo data'
  print *,' TOPO_Z min=',minval(TOPO_Z),' max=',maxval(TOPO_Z)

  !---- Extend the input grid one point in each direction
  !     so all possible lat-lons are included
  TOPO_Z(0,1:ny_topo) = TOPO_Z(nx_topo,1:ny_topo)
  TOPO_Z(nx_topo+1,1:ny_topo) = TOPO_Z(1,1:ny_topo)
  TOPO_Z(:,0) = TOPO_Z(:,1)
  TOPO_Z(:,ny_topo+1) = TOPO_Z(:,ny_topo)

  print *,' Extended TOPO_Z min=',minval(TOPO_Z),' max=',maxval(TOPO_Z)
...
!!$  OCN_CORNER_LAT(4,:,:) = Y_SG(1::2,3::2)
  do j=1,ny_o
     j_l = 1 + (j-1)*2    ! j lower 1,3,5, ...
     j_u = j_l + 2         ! j upper 3,5,7, ...
     if (j_l > ny_topo .OR. j_u > ny_topo ) then
        print * , ' topo indexing out of bounds in j',j,j_l,j_u,ny_topo
        stop
     endif
     do i=1,nx_o
        i_l = 1 + (i-1)*2   ! i left 
        i_r = i_l + 2   ! i right
        if (i_l > nx_topo .OR. i_r > nx_topo ) then
           print * , ' topo indexing out of bounds in i',i,i_l,i_r,nx_topo
           stop
        endif

        OCN_CORNER_LON(1,i,j) = X_SG(i_l,j_l)    ! lower left
        OCN_CORNER_LAT(1,i,j) = Y_SG(i_l,j_l)
        OCN_CORNER_LON(2,i,j) = X_SG(i_r,j_l)    ! lower right
        OCN_CORNER_LAT(2,i,j) = Y_SG(i_r,j_l)
        OCN_CORNER_LON(3,i,j) = X_SG(i_r,j_u)    ! upper right
        OCN_CORNER_LAT(3,i,j) = Y_SG(i_r,j_u)
        OCN_CORNER_LON(4,i,j) = X_SG(i_l,j_l)    ! upper left
        OCN_CORNER_LAT(4,i,j) = Y_SG(i_l,j_l)
     enddo
  enddo
        ...
        do jsub=1,ny_sub
           ...
           do isub=1,nx_sub
              ...
              if ( rlat < TOPO_LAT(0) .OR. rlat > TOPO_LAT(ny_topo+1)) then
                 print *,' Invalid search lat/lon from rlat'
...
!!$              topo_j = floor( (rlat - TOPO_LAT(0))/dlat_topo )+1
              ...
              if ( topo_i < 0 .OR. topo_i > nx_topo+1) then
                 print *,' Invalid search longitude from topo_i'
                 print *,' topo_i,topo_j = ',topo_i,topo_j
                 print *,' subpoint i,j, = ',isub, jsub
                 print *,' corner lon = ',OCN_CORNER_LON(:,ocn_i,ocn_j)
                 print *,' corner lat = ',OCN_CORNER_LAT(:,ocn_i,ocn_j)
                 print *,' rlon = ',rlon
              ...
              if ( topo_j < 0 .OR. topo_j > ny_topo+1) then
                 print *,' Invalid search latitude from topo_j'
                 print *,' topo_i,topo_j = ',topo_i,topo_j
                 print *,' model i,j, = ',ocn_i, ocn_j
                 print *,' subpoint i,j, = ',isub, jsub
        ...
        if (nsum /= 0) then
           ...
           elseif ( nsumo == 1) then
              OCN_D_MEDIAN(ocn_i,ocn_j) = rbuffer(1)
           endif
